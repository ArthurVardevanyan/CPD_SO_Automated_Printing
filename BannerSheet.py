...
def banner_sheet(JOB_INFO, OUTPUT_PATH):
    ...
    with open("".join([OUTPUT_PATH, JOB_INFO.get('Order Number', False), ' Banner ', JOB_INFO.get('First Name', False), ' ', JOB_INFO.get('Last Name', False), '.ps']), 'wb') as outfile:
        # Export PJL Lines
        ...
        for key, value in JOB_INFO.items():
            ...
            if(key != 'Files' and key != 'Ran' and key != 'Account ID' and key != 'Order Number' and key != 'Building'):
                outfile.write(str.encode("".join(['75 ', str(vertical_position),
    ...
    return "".join([OUTPUT_PATH, JOB_INFO.get('Order Number', False), ' Banner ', JOB_INFO.get('First Name', False), ' ', JOB_INFO.get('Last Name', False), '.ps'])
